...
const loginUser=async(email:string,password:string)=>{
    var data:any
    const checkIs=await conexion.query("SELECT p.idProfesor,p.idRol,p.pass,p.nombre_profesor,p.apellido_profesor,p.usuario,r.rol FROM tbProfesor p INNER JOIN tbRol r ON r.idRol=p.idRol WHERE p.usuario=? and p.estatus='1' GROUP BY p.idProfesor",[email]);
    if(checkIs==''){
        ...
        if(checkIs==''){
            const checkIs=await conexion.query("SELECT a.idAlumno,a.idRol,a.pass,a.nombres_alumno,a.apellidos_alumno,a.usuario,r.rol FROM tbAlumno a INNER JOIN tbRol r ON r.idRol=a.idRol WHERE a.usuario=? and a.activo='1' GROUP BY a.idAlumno",[email]);
            if(checkIs==''){
                ...
                const token=generateToken(idUsuario,idRol,nombres_alumnos,apellidos_alumnos,rol);
                data={
                    token,
                    user:datos
                }
            ...
            let datos = Object.values(results);
            //fin de data en modo objeto
            const token=generateToken(idUsuario,idRol,nombre_tutor,apellido_tutor,rol);
            data={
                token,
                user:datos
            }
        ...
        let datos = Object.values(results);
        //fin de data en modo objeto
        const token=generateToken(idUsuario,idRol,nombre_profesor,apellido_profesor,rol);
        data={
            token,
            user:datos
        }
