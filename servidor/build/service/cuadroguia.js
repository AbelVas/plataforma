...
exports.notasalumnosCursoFinalService = exports.cursosGradoCuadroGuiaService = exports.alumnosGradoService = exports.notasalumnosFinalService = exports.actividadesCursoGradoService = exports.GradoCursoSeccionService = exports.cursosGradoCuadroFinalService = void 0;
const database_1 = __importDefault(require("../config/database"));
const alumnosGradoService = (idGrado) => __awaiter(void 0, void 0, void 0, function* () {
    const dataAlumnos = yield database_1.default.query("SELECT idAlumno,CONCAT(nombres_alumno,', ',apellidos_alumno)as alumno FROM `tbAlumno` WHERE idGrado=? order by apellidos_alumno", [idGrado]);
    return dataAlumnos;
});
exports.alumnosGradoService = alumnosGradoService;
const GradoCursoSeccionService = (idGrado, idCurso) => __awaiter(void 0, void 0, void 0, function* () {
    const datos = yield database_1.default.query("SELECT g.nombre_grado,s.seccion,c.nombre_curso, CONCAT(p.apellido_profesor,', ',p.nombre_profesor) as profesor FROM ((tbGrado g INNER JOIN tbSeccion s ON g.idSeccion=s.idSeccion)INNER JOIN tbCurso c ON c.idGrado=g.idGrado)INNER JOIN tbProfesor p ON p.idProfesor=c.idProfesor where g.idGrado=? AND c.idCurso=?", [idGrado, idCurso]);
    return datos;
});
exports.GradoCursoSeccionService = GradoCursoSeccionService;
const actividadesCursoGradoService = (idCurso, idUnidad) => __awaiter(void 0, void 0, void 0, function* () {
    const actividadesCurso = yield database_1.default.query("SELECT idDetalleActividad,nombre_actividad from tbDetalleActividad WHERE idCurso=? and idUnidad=? order by fecha_entrega", [idCurso, idUnidad]);
    return actividadesCurso;
});
exports.actividadesCursoGradoService = actividadesCursoGradoService;
const notasalumnosFinalService = (idCurso, idUnidad, idGrado) => __awaiter(void 0, void 0, void 0, function* () {
    var consulta = {};
    var idActividades = [];
    var temp = [];
    var idAlumnos = [];
    ...
    const actividadesCurso = yield database_1.default.query("SELECT idDetalleActividad,nombre_actividad from tbDetalleActividad WHERE idCurso=? and idUnidad=? order by fecha_entrega", [idCurso, idUnidad]);
    const actividadesArray = Object.values(actividadesCurso);
    ...
    switch (idActividades.length) {
        case 1:
            for (let i = 0; i < idAlumnos.length; i++) {
                consulta = yield database_1.default.query("SELECT CONCAT(a.apellidos_alumno,', ',a.nombres_alumno) as alumno,fcursoactividad(" + idCurso + "," + idAlumnos[i] + "," + idActividades[0] + "," + idUnidad + ") as cero FROM tbAlumno a WHERE idAlumno=" + idAlumnos[i] + "");
                const notaArray = Object.values(consulta);
                temp[i] = {
        ...
        case 2:
            for (let i = 0; i < idAlumnos.length; i++) {
                consulta = yield database_1.default.query("SELECT CONCAT(a.apellidos_alumno,', ',a.nombres_alumno) as alumno,fcursoactividad(" + idCurso + "," + idAlumnos[i] + "," + idActividades[0] + "," + idUnidad + ") as cero,fcursoactividad(" + idCurso + "," + idAlumnos[i] + "," + idActividades[1] + "," + idUnidad + ") as uno FROM tbAlumno a WHERE idAlumno=" + idAlumnos[i] + "");
                const notaArray = Object.values(consulta);
                temp[i] = {
                    alumno: notaArray[0].alumno,
                    uno: notaArray[0].cero,
...
const cursosGradoCuadroFinalService = (idGrado) => __awaiter(void 0, void 0, void 0, function* () {
    const cursos = yield database_1.default.query("SELECT idCurso,abreviatura,nombre_curso FROM tbCurso WHERE idGrado=? and consolidado_anual=1", [idGrado]);
    return cursos;
});
exports.cursosGradoCuadroFinalService = cursosGradoCuadroFinalService;
//consolidado cursos del grado por bimestre
const notasalumnosCursoFinalService = (idGrado, idUnidad) => __awaiter(void 0, void 0, void 0, function* () {
    var consulta = {};
    var idCursos = [];
    var temp = [];
    var idAlumnos = [];
    //Obtengo los cursos
    const cursos = yield database_1.default.query("SELECT idCurso,abreviatura,nombre_curso FROM tbCurso WHERE idGrado=? and consolidado_bimestre=1", [idGrado]);
    ...
    switch (idCursos.length) {
        case 1:
            for (let i = 0; i < idAlumnos.length; i++) {
                consulta = yield database_1.default.query("SELECT CONCAT(a.apellidos_alumno,', ',a.nombres_alumno) as alumno,fmateria(" + idUnidad + "," + idCursos[0] + "," + idAlumnos[i] + ") as cero FROM tbAlumno a WHERE idAlumno=" + idAlumnos[i] + "");
                const notaArray = Object.values(consulta);
                temp[i] = {
                    alumno: notaArray[0].alumno,
                    uno: notaArray[0].cero,
        ...
        case 2:
            for (let i = 0; i < idAlumnos.length; i++) {
                consulta = yield database_1.default.query("SELECT CONCAT(a.apellidos_alumno,', ',a.nombres_alumno) as alumno,fmateria(" + idUnidad + "," + idCursos[0] + "," + idAlumnos[i] + ") as cero,fmateria(" + idUnidad + "," + idCursos[1] + "," + idAlumnos[i] + ") as uno FROM tbAlumno a WHERE idAlumno=" + idAlumnos[i] + "");
                const notaArray = Object.values(consulta);
                temp[i] = {
                    alumno: notaArray[0].alumno,
                    uno: notaArray[0].cero,
        ...
        case 3:
            for (let i = 0; i < idAlumnos.length; i++) {
                consulta = yield database_1.default.query("SELECT CONCAT(a.apellidos_alumno,', ',a.nombres_alumno) as alumno,fmateria(" + idUnidad + "," + idCursos[0] + "," + idAlumnos[i] + ") as cero,fmateria(" + idUnidad + "," + idCursos[1] + "," + idAlumnos[i] + ") as uno,fmateria(" + idUnidad + "," + idCursos[2] + "," + idAlumnos[i] + ") as dos FROM tbAlumno a WHERE idAlumno=" + idAlumnos[i] + "");
                const notaArray = Object.values(consulta);
                temp[i] = {
                    alumno: notaArray[0].alumno,
                    uno: notaArray[0].cero,
...
exports.notasalumnosCursoFinalService = notasalumnosCursoFinalService;
const cursosGradoCuadroGuiaService = (idGrado) => __awaiter(void 0, void 0, void 0, function* () {
    const cursos = yield database_1.default.query("SELECT idCurso,abreviatura,nombre_curso FROM tbCurso WHERE idGrado=? and consolidado_bimestre=1", [idGrado]);
    return cursos;
});
exports.cursosGradoCuadroGuiaService = cursosGradoCuadroGuiaService;
